import { VIC_MODELS, VIC_FORMAT, VIC_OPTION } from '../constants';

const minWidth = (model) => {
    if (VIC_MODELS.TYPE_287.includes(model)) return 8;
...
const maxWidth = (parentState) => {
    ...
    if (VIC_MODELS.TYPE_250.includes(model)) {
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 2464 : 0;
        }
        if (bitDepth === 10 && format === VIC_FORMAT.CXP3x2) {
            return 1232;
        }
        if (bitDepth === 10 && format === VIC_FORMAT.CXP6x1) {
            return 1232;
        }
        if (bitDepth === 12 && format === VIC_FORMAT.CXP2x2) {
            return 2200;
        }
        return 2464;
        ...
        return 2064;
    } else if (VIC_MODELS.TYPE_253.includes(model) || VIC_MODELS.TYPE_255.includes(model)) {
        if (subSampling) {
            if (bin0) return 2056;
            return 0;
        }
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 4112 : 0;
        }
        if (bin2) {
            return 2056;
        }
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 728 : 0;
        }
        if (bin2) {
            return 728;
        }
        ...
        if (cameraOption !== VIC_OPTION.NONE) return 0;
...
const maxHeight = (parentState) => {
    ...
    if (VIC_MODELS.TYPE_250.includes(model)) {
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 1028 : 0;
        }
        return 2056;
    } else if (VIC_MODELS.TYPE_252.includes(model)) {
        if (subSampling) {
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 772 : 0;
        }
        if (bitDepth === 12 && format === VIC_FORMAT.CXP6x2) {
            return 1080;
        }
        if (bitDepth === 12 && format === VIC_FORMAT.CXP5x2) {
            return 1080;
        }
        return 1544;
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 1504 : 0;
        }
        if (bin2) {
            return 1504;
        }
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 1088 : 0;
        }
        if (bin2) {
            return 1088;
        }
        ...
        if (binv) {
            return VIC_MODELS.TYPE_MONO.includes(model) ? 544 : 0;
        }
        if (bin2) {
            return 544;
        }
